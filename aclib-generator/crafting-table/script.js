...
fetch('../idlist.json').then(res => res.json()).then(idlist => {
  ...
  const inputs = recipeInterface.querySelectorAll('.input > span');
  const output = recipeInterface.querySelector('.output > span.out');
  ...
  const craftingModeSelect = detailsContainerContent.querySelectorAll('.default input.craftingMode');
  ...
  if(projectId && projectRecipeId) {
    ...
    if(!localStorage.getItem('aclib-current')) localStorage.setItem('aclib-current', '{"craftingTable":{}}')
    if(!JSON.parse(localStorage.getItem('aclib-current')).craftingTable) localStorage.setItem('aclib-current', JSON.stringify({...JSON.parse(localStorage.getItem('aclib-current')), craftingTable: {}}));
    recipe = JSON.parse(localStorage.getItem('aclib-current')).craftingTable;
  }
  ...
  inputs.forEach(input => input.addEventListener('click', ev => {
    // if(ev.ctrlKey) return;
    activeItem = input.dataset.item;
    [...inputs, output].forEach(e => e.classList.remove('active'));
  ...
  craftingModeSelect.forEach(e => e.addEventListener('change', ev => {
    recipe.craftingMode = ev.target.dataset.craftingmode;
    updateLocalStorage();
  }));
  ...
  showJSONBtn.addEventListener('click', () => {
    ...
    //
    //
    const json = craftingTableGenerate(recipe, false);
    console.log(json);
  ...
  downloadPackBtn.addEventListener('click', () => {
    const json = craftingTableGenerate(recipe, true);
    if(json == false) return;
    ...
    JSZipUtils.getBinaryContent("../imgs/pack-icons/crafting_table.png", function (err, data) {
      if(err) {
  ...
  [...inputs, output].forEach(e => {
    ...
    e.addEventListener('drop', ev => {
      ...
      ev.target.classList.remove('dragover');
      const from = [...inputs, output].find(e => /dragging/.test(e.classList));
      const to = ev.target;
  ...
  window.addEventListener('keydown', ev => {
    ...
    if(ev.key == 'Backspace' || ev.key == 'Delete') {
      ev.preventDefault();
      if(activeItem != '') {
        if(/^in\d$/.test(activeItem)) detailsContainerContent.querySelector('.input .delete').click();
  ...
  function refreshDetails() {
    ...
    } else if(/^in\d$/.test(oldActiveItem)) {
  ...
  function reloadRecipe() {
    ...
    [...inputs, output].forEach(e => {
      if(recipe[e.dataset.item]) {
        const item = /^#/.test(recipe[e.dataset.item].id) && /^in\d$/.test(e.dataset.item) ? {name: 'Item Tag', link: '../imgs/item_tag.png'} : (getItemById(/:/.test(recipe[e.dataset.item].id) ? recipe[e.dataset.item].id.replace(/ /g, '_') : `minecraft:${recipe[e.dataset.item].id.replace(/ /g, '_')}`));
  ...
  function updateLocalStorage() {
    ...
    else localStorage.setItem('aclib-current', JSON.stringify({...JSON.parse(localStorage.getItem('aclib-current')), craftingTable: recipe}));
  }
